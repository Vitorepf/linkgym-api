...
The domain language is Portuguese (Pessoa, Time, Vínculo). The Postgres schema uses English identifiers (`people`, `studios`, `bonds`) so Go, SQL tooling, and the AI stay on one spelling. Mapping lives in `CONTEXT.md` and in comments on each table.
