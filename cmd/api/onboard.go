...
func (a *api) commitmentPut(w http.ResponseWriter, r *http.Request) {
	sess := sessionFrom(r)
	var req onboard.Commitment
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeError(w, http.StatusBadRequest, "json_invalido")
		return
	}
	if err := a.onboard.Commit(r.Context(), sess.Person.ID, req.DaysPerWeek); err != nil {
		if errors.Is(err, onboard.ErrInvalid) {
			writeError(w, http.StatusBadRequest, err.Error())
			return
		}
